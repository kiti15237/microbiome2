...
aut<-read.table("/Users/mmdavid/Documents/argonne_data/microbiome_analysis1_19samples/differences/class/aut.txt", header = T)
controls<-read.table("/Users/mmdavid/Documents/argonne_data/microbiome_analysis1_19samples/differences/class/control.txt", header = T)

diffall<-aut-controls
library(ade4)
pcadiff<-dudi.pca(diffall)
#scatter(pcadiff)
plot(pcadiff$co, col = Cond)
...
heatmap_test<-pheatmap(select, clustering_method ="ward")
